...
Capture the developer's goal as a structured issue so the developer and agent share a clear definition of success before implementation begins. A well-defined issue prevents wasted effort on the wrong problem.
...
- All traceability links complete (Pain←Situation, Benefit←Pain, AS←Benefit)
...
2. Ask clarifying questions until goal, scope, and target users are clear — ambiguity at this stage compounds into wrong deliverables later
3. Identify the concrete situation, affected users, and problems they face

### 2. Draft

1. Write the Situation section from gathered facts — only include observed circumstances, not assumptions
2. Write the Pain table — derive each pain from the Situation so the developer can trace every problem back to observed facts
3. Write the Benefit table — link each benefit to a Pain so every proposed improvement addresses a real problem
4. Write the Acceptance Scenarios tables — group by target user and link each scenario to a Benefit so coverage per user is visible at a glance
...
- If requirements are unclear, ask the developer before revising — guessing risks wasted effort
