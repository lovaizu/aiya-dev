...
Verify that each AS is met from the target user's perspective.
...
3. Prefer execution over inspection:
   ...
   b. **Inspect**: Only if execution is impossible — explain why, then inspect the implementation
4. Record the judgment in the format above
...
6. No AS has status NG

## Iterate

- If any AS is NG, fix the issue and re-evaluate the failed scenario
